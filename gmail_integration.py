...
class GmailService:
    def __init__(self, credentials_file='credentials.json', token_file=None):
    ...
    def list_authenticated_accounts():
        import glob
        tokens = glob.glob('token_*.pickle')
        accounts = []
        for t in tokens:
            basename = os.path.basename(t)
            if basename.startswith("token_") and basename.endswith(".pickle"):
                email = basename[6:-7]  # extract email address from token_email.pickle
    ...
    def authenticate(self, target_email=None):
        """Authenticate and create Gmail API service"""
        # Migrate old token.pickle if it exists
        if os.path.exists('token.pickle'):
            try:
                with open('token.pickle', 'rb') as legacy_token:
                    legacy_creds = pickle.load(legacy_token)
                legacy_service = build('gmail', 'v1', credentials=legacy_creds, static_discovery=False)
                profile = legacy_service.users().getProfile(userId='me').execute()
                email_address = profile.get('emailAddress')
                if email_address:
                    os.rename('token.pickle', f'token_{email_address}.pickle')
            except Exception:
                try:
        ...
        # Determine the token file name based on input email
        if target_email:
            self.token_file = f'token_{target_email}.pickle'
        else:
        ...
        if not creds or not creds.valid:
            ...
            # Get the email to save token dynamically
            email_address = self.get_user_email()
            if email_address:
                self.token_file = f'token_{email_address}.pickle'
